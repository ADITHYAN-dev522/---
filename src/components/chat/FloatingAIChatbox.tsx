...
export default function FloatingAIChatbox() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: "assistant",
      content:
        "Hello! I'm your AI Security Analyst. I can help you analyze threats, vulnerability data, and logs from your SOC. How can I assist you today?",
      timestamp: new Date(),
    },
  ]);
  ...
  const handleSend = async () => {
    ...
    const messageToSend = input; // preserve current input
    setInput("");
    setLoading(true);

    try {
      // Fetch latest scan results
      const latestScanRes = await fetch("http://localhost:8000/api/scan/results");
      const latestScan = await latestScanRes.json();

      // Send to AI backend
      const res = await fetch("http://localhost:8000/api/ai/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ message: messageToSend, context: latestScan }),
      });
      ...
      const errorMessage: Message = {
        ...
        role: "assistant",
        content: "AI backend is unreachable or failed. Please make sure your Qwen model is running.",
        timestamp: new Date(),
      };
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <motion.div
                        ...
                        transition={{ duration: 2, repeat: Infinity }}
                      />
                      <p className="text-xs text-white/60">{loading ? "Processing..." : "Online"}</p>
                    </div>
                  </div>
                </div>
                <Button
